    ...
    async fn spawn_internal(args: SessionSpawnArgs) -> CodexResult<(Arc<Self>, SessionIo)> {
            ...
            && config.config_lock_save_fields_resolved_from_model_catalog
        {
            let use_model_defaults = config.features.enabled(Feature::TokenBudget)
                && !self::token_budget::has_explicit_settings(&config);
            let token_budget = self::token_budget::resolve_token_budget(
                config.token_budget.as_ref(),
                use_model_defaults,
                &model_info,
            );
            Arc::make_mut(&mut config).token_budget = token_budget;
        }
...
impl Session {
    ...
    pub(crate) async fn record_conversation_items(
        ...
        self.record_conversation_items_then(turn_context, model_info, items, || {})
            .await;
    }
