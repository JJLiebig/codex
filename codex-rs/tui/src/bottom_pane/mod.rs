...
impl BottomPane {
    ...
    pub(crate) fn as_renderable_with_options<'a>(
        ...
        self.as_renderable_with_status_override(options, /*status_override*/ None)
    }
        ...
        if let Some(view) = self.active_view() {
            ...
            if let Some(status) = self.status_widget() {
                let status: RenderableItem<'_> = match status_override {
                    Some(header) => RenderableItem::Owned(Box::new(
                        status.as_renderable_with_header(&self.status_timer, header),
                    )),
                    None => RenderableItem::Owned(Box::new(status.with_timer(&self.status_timer))),
                };
