...
struct IpcSpawnedProcess {
    ...
    job: Arc<JobObject>,
    preserve_descendants_on_exit: bool,
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    ...
    let (pi, job, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        ...
        let spawned_pipes: PipeSpawnHandles = spawn_process_with_pipes(
            ProcessExecutionMode::Token(h_token.raw()),
            &req.command,
            &effective_cwd,
            &req.env,
            stdin_mode,
            ...
            if req
                .command
                .get(1)
                .is_some_and(|arg| arg == codex_plus_plus::FS_HELPER_ARG)
            {
...
pub fn main() -> Result<()> {
    ...
    let preserve_descendants_on_exit = ipc_spawn.preserve_descendants_on_exit;
    ...
    } else if preserve_descendants_on_exit {
    ...
    if child_stopped {
        if out_thread.is_some_and(|thread| thread.join().is_err()) {
